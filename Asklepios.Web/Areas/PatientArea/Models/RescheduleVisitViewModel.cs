...
namespace Asklepios.Web.Areas.PatientArea.Models
{
    ...
    {
        public IQueryable<Visit> _allVisitsList;
        public IQueryable<Visit> AllVisitsList 
        {
            get { return _allVisitsList; }
            set { _allVisitsList = value; }
        }
        ...
        public List<VisitCategory> GetVisitCategories
        {
            get
            {
                ...
                if (AllVisitsList.Count() > 0)
                {
                    List<VisitCategory> categories =  AllVisitsList.Where(d=>d.VisitCategoryId==RescheduledVisit.VisitCategoryId).Select(c => c.VisitCategory).Distinct().ToList();
                    return categories;
                }
                else
                {
        ...
        private List<Visit> FilterVisits()
        {
            ...
            if (SelectedVisitCategoryId != null)
            {
                if (long.TryParse(SelectedVisitCategoryId, out long lid))
                {
                    if (lid>0)
                    {
                        filteredVisits = filteredVisits.Where(c => c.VisitCategory.Id == int.Parse(SelectedVisitCategoryId)).AsQueryable();
                        if (filteredVisits == null)
                        {
                            return null;
                        }
